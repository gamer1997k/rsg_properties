...
AddEventHandler('rsg_properties:server:buyhouse', function(price, id)
	...
	if checkowned == false then
		if currentBank > price then
			...
			-- notify player
			TriggerClientEvent('rsg:Notify', src, 8, 'House Purchased', 5000, 'you now own this house', 'satchel_textures', 'satchel_nav_sell', 'COLOR_WHITE')
			Wait(5000)
			TriggerClientEvent('rsg:Notify', src, 8, 'Keys Dispatched', 5000, 'you should now have the keys', 'inventory_items', 'folder_kit_keychain', 'COLOR_WHITE')
		else
			TriggerClientEvent('rsg:Notify', src, 9, 'you don\'t have enough money in your bank!', 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	else
		TriggerClientEvent('rsg:Notify', src, 9, 'you already own a property!', 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
	end
end)
...
AddEventHandler('rsg_properties:updatedoorsv', function(doorID, state, cb)
	...
	TriggerClientEvent('rsg:Notify', src, 9, Lang:t("error.nokey"), 2000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		return
	else
